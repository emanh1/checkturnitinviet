...
onMounted(async () => {
  ...
  try {
    ...
    const payment = await $fetch(`/api/payments/status?transactionId=${transactionId}`)
      ...
      <div v-else class="text-center py-12">
        <div class="mb-6 flex justify-center">
          <div class="rounded-full bg-emerald-100 p-4 dark:bg-emerald-900/30">
            <UIcon name="i-lucide-check-circle-2" class="h-12 w-12 text-emerald-600 dark:text-emerald-400" />
          </div>
        </div>

        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Thanh toán thành công!</h1>
        ...
        <div class="mt-8 rounded-lg bg-slate-50 p-6 dark:bg-slate-900">
          <p class="text-sm text-slate-600 dark:text-slate-400">credits hiện có</p>
          <p class="mt-2 text-4xl font-bold text-slate-900 dark:text-white">{{ profile?.credits ?? 0 }}</p>
        </div>
